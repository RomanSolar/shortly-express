...
class Users extends Model {
  ...
  /**
   * Authenticates an existing user record with the given username and password.
   * @param {Object} user - The user object.
   * @param {string} user.username - The user's username.
   * @param {string} user.password - The plaintext password.
   * @returns {Promise<Object>} A promise that is fulfilled with the ???????;
   */
  login({ username, password }) {
    return this.get({ username })
      .then(user => {
        if (user && this.compare(password, user.password, user.salt)) {
          return user.id;
        }
      });
  }
