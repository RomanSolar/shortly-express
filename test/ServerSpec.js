...
describe('', function() {
  ...
  beforeEach(function(done) {
    ...
    /*************************************************************************************/
    db = mysql.createConnection(require('../server/env/config.js').db);
  ...
  describe('Account Login:', function() {
    ...
    it('Logs in users with their userId', function(done) {
      var options = {
        'method': 'POST',
        'uri': 'http://127.0.0.1:4568/login',
        'json': {
      ...
      request(options, function(error, res, body) {
        if (error) { return done(error); }
        var query = 'SELECT * FROM sessions WHERE userId = ?';
        db.query(query, [1], function(err, results) {
          if (error) { return done(error); }
  ...
  describe('Express Middleware', function() {
    ...
    describe('Session Parser', function(done) {
